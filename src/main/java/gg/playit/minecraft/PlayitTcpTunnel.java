...
public class PlayitTcpTunnel {
    ...
    private class TunnelConnectionHandler extends SimpleChannelInboundHandler<ByteBuf> {
        ...
        private boolean addChannelToMinecraftServer() {
            gg.playit.channel.ChannelInserter inserter = gg.playit.channel.ChannelInserterProvider.get(server);
            if (inserter == null) {
                log.info("no ChannelInserter for this Minecraft version");
                return false;
            }

            int rateLimit = 0;
            try {
                Object minecraftServer = getMinecraftServerForRateLimit();
                if (minecraftServer != null) {
                    Integer rl = getRateLimitFromMinecraftServer(minecraftServer);
                    if (rl != null) rateLimit = rl;
                }
            } catch (Exception ignored) {
            }

            return inserter.insertChannel(server, tunnelChannel, trueIp, connectionTimeoutSeconds, rateLimit);
        }

        private Object getMinecraftServerForRateLimit() {
            try {
                Class<?> craftServer = server.getClass();
                if (craftServer.getName().startsWith("org.bukkit.craftbukkit")) {
                    Object mc = craftServer.getMethod("getServer").invoke(server);
                    if (mc != null && mc.getClass().getName().equals("net.minecraft.server.MinecraftServer")) {
                        return mc;
                    }
                }
            } catch (Exception ignored) {
        ...
        private Integer getRateLimitFromMinecraftServer(Object minecraftServer) {
            try {
                var m = minecraftServer.getClass().getMethod("getRateLimitPacketsPerSecond");
                return (Integer) m.invoke(minecraftServer);
            } catch (Exception e) {
                return null;
            }
        }
